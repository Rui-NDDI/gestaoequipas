...
### 3. Securing Credentials via settings.php (Recommended)

Storing credentials in the Drupal database means they can be exposed in
a database breach. For production sites, override the sensitive credential
settings in `settings.php` so they are never written to the database.

**Option A: Using environment variables (most secure)**
...
Then export the variables in your server environment (e.g. `.env`, Docker,
CI/CD pipeline, or platform environment config):
...
**Option B: Hard-coded values in settings.php**

```php
$config['social_auth_entra_id.settings']['client_id'] = 'your-client-id';
$config['social_auth_entra_id.settings']['client_secret'] = 'your-client-secret';
$config['social_auth_entra_id.settings']['tenant_id'] = 'your-tenant-id';
```

> **Note:** When a credential is overridden in `settings.php`, the
> corresponding field on the settings page will be disabled and a notice
> will be shown. The overridden value is never saved back to the database.
